...
if 1:
    ...
    for i in xrange(n_bins):
        if i< n_bins-1:
        #print i
            h1.SetBinContent(i+1,dfx.recum.iloc[i+1])
            #h1.SetBinError(i+1,2) 
            #h1.SetBinError(i+1,dfx.rc_w2_sq.iloc[i+1])
    
        #h2.SetBinContent(i+1,2)
        h2.SetBinContent(i+1,dfx.ws.iloc[i])
        h2.SetBinError(i+1,0.0813976)
        #h2.SetBinError(i+1,dfx.w2s.iloc[i])

    h1.SetBinContent(n_bins+1,0)
    h1.SetBinError(n_bins+1,0)    
    """
